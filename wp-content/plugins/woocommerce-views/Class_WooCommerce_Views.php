...
class Class_WooCommerce_Views {
	...
	public function __construct() {
		...
		// Since Views 2.3.0 groups and items get registered differently in the Fields and Views dialog
		add_action( 'init', array(&$this,'wpv_woo_register_shortcodes_dialog_groups'), 15 );
...
<?php 
	...
	**/
	...
	public function add_to_cart_buy_or_select_closures_listing($argument_one=null,$argument_two=null) {
		...
		if ($is_using_revised_wc) {
			//Check product type
			$product_type_passed	= $this->wc_views_get_product_type( $argument_two );			
			if ( 'variable' === $product_type_passed ) {
				...
				if (isset($atts['show_variation_options'])) {
				   ...
				   if ('yes' == $show_variation_options) {
				   	//User wants to display variation options on listing pages
					   	if('variable' == $this->wc_views_get_product_type( $product )  ) {
	...
	global $post, $wpdb, $woocommerce;
		...
		if ( 'product' == $post->post_type ) {        
	        ...
	        if (is_product()) {						
				...
				if ('simple' == $this->wc_views_get_product_type( $product ) ) {
								
					do_action( 'woocommerce_simple_add_to_cart');
				
				} elseif ('variable' == $this->wc_views_get_product_type( $product ) ) {
	 				    
					do_action( 'woocommerce_variable_add_to_cart'); 
					
				} elseif ('grouped' == $this->wc_views_get_product_type( $product ) ) {
					
					do_action( 'woocommerce_grouped_add_to_cart');
					
				} elseif ('external' == $this->wc_views_get_product_type( $product ) ) {
	...
	*/
	...
	public function wpv_woo_register_shortcodes_dialog_groups() {
		...
		if ( isset( $post ) ) {
			if (
				...
				|| ! in_array( $post->post_type, $associated_posttypes )
			) {
		...
		$group_data	= array(
			...
			'fields'	=> array(
				'wpv-woo-buy-or-select' => array(
					...
					'callback'	=> "wcviews_insert_wpv_woo_buy_or_select(); return false;"
				),
				'wpv-woo-product-price' => array(
				...
				'wpv-woo-buy-options' => array(
					...
					'callback'	=> "wcviews_insert_wpv_woo_buy_options(); return false;"
				),
				'wpv-woo-product-image' => array(
					'name'		=> __( 'Product image', 'woocommerce_views' ),
					'shortcode'	=> 'wpv-woo-product-image',
					'callback'	=> "wcviews_insert_wpv_woo_product_image(); return false;"
				),
				'wpv-add-to-cart-message' => array(
				...
				'wpv-woo-display-tabs' => array(
					...
					'callback'	=> "wcviews_insert_wpv_woo_display_tabs(); return false;"
				),
				'wpv-woo-onsale' => array(
				...
				'wpv-woo-productcategory-images' => array(
					...
					'callback'	=> "wcviews_insert_wpv_woo_productcategory_images(); return false;"
				),
				'wpv-woo-show-upsell-items' => array(
  	   ...
  	   //Step1, we need to ensure 'WooCommerce' plugin is activated
  		...
  		if ( is_object( $woocommerce )) {
  			...
  			if (isset( $view_settings['post_type']) ) {  				
  				...
  				/**
  				 * WooCommerce Views 2.7.3: Special cases in archive implementations
  				 * When $post_types are ambiguous.
  				 */
  				$inject_callback	= false;
  				if ( in_array('product', $post_types ) ) {
  					//Scenario 1: When querying products in standalone Views
  					$inject_callback	= true;
  				} else {
  					//Scenario 2: Could be archive implementations, checking further..
  					$queried_object_loaded	= get_queried_object();
  					if ( ( is_object( $queried_object_loaded ) ) && ( isset( $queried_object_loaded->query_var ) ) ) {
  						$query_var	= $queried_object_loaded->query_var;
  						if ( 'product'	=== $query_var ) {
  							//We are querying WooCommerce products here.
  							$inject_callback	= true;
  						}
  					}
  				}
  				if ( true === $inject_callback ) {  					
  				    //Product post type set  				    	
  				    //Step3,we need to check if pagination is enabled to AJAX  	
  				    /** WooCommerce Views 2.6.9: woocommerceviews-78 Check for both old and new handler 
  				     *  For backward compatibility
  				     * */
  				    if ( ( isset( $view_settings['pagination']['mode'] ) ) || ( isset( $view_settings['pagination']['type'] ) ) ) { 
  				    	$pagination_mode	= '';
  				    	if ( isset( $view_settings['pagination']['mode'] ) ) {
  				    		$pagination_mode	=	$view_settings['pagination']['mode'];
  				    	} elseif ( isset( $view_settings['pagination']['type'] ) ) {
  				    		$pagination_mode	=	$view_settings['pagination']['type'];
  				    	}    				    	
  				    	if ( ( 'paged' == $pagination_mode ) || ( 'ajaxed' == $pagination_mode ) || ( 'rollover' == $pagination_mode ) ) {  				    		
  				    		//It's paginated
