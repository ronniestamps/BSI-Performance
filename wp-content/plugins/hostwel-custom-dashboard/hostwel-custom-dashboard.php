<?php
/**
 * @package hostwel-custom-dashboard
 * @version 1.3
 */
/*
Plugin Name: Hostwel Custom Dashboard
Plugin URI: http://hostwel.com
...
Version: 1.3.1
Author: Hostwel - Ronnie Stamps
Author URI: http://hostwel.com
...
function cdw_reg_menu(){
	add_options_page( __('Custom Dashboard', 'DashboardWidgets'), __('Custom Dashboard', 'DashboardWidgets'), 'administrator', 'dashboard-widgets', 'cdw_DashboardWidgets'); 
}
    ...
    $widget_button_class = "main_bashboard_widget_button";
		...
		foreach($data as $item){ 
			...
			if(!isset($item[$role]) ||  ($item[$role] != 'checked') ) continue;
                ...
                }else if(strpos($iconItem , 'fa-') !==false) {
                    $icon = '<i class="fa '.$iconItem.'"></i>';
                } else {
                    $icon = '<img src="'.$iconItem.'" />';
                }

                if($item['title'] == 'new row') { 
                    echo '<br>';
                    $newRow = "new-row";
				...
				echo '<div class="'.$widget_button_class.' '.$newRow.'">
					<a href="'.$link.'">
						'.$icon.'
						<h3>'.__($item['title']).'</h3>
					</a>
